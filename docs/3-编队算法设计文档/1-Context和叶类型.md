...
class FormContextS:                        # 编队算法上下文（跨拍常驻、原地迭代）
    cmd: FormSnapshotS = field(default_factory=FormSnapshotS)      # 编队指令
    ...
    leaderCmd: MotionProfS = field(default_factory=MotionProfS)   # 长机(leader)指令
    leaderState: MotionProfS = field(default_factory=MotionProfS) # 长机状态
    leaderErr: MotionProfS = field(default_factory=MotionProfS)   # 长机误差
    selfCmd: MotionProfS = field(default_factory=MotionProfS)     # 本机指令
    selfState: MotionProfS = field(default_factory=MotionProfS)   # 本机状态
    selfErr: MotionProfS = field(default_factory=MotionProfS)     # 本机误差
    selfAccCmd: AccInEarthS = field(default_factory=AccInEarthS)  # 跟踪算法输出的加速度指令
```
